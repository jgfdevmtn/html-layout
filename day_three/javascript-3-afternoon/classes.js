...
john = new Employee('John','Fisher', 'jfisher@gmail.com', 21)
...
class Manager {
	...
	hire(employee) {return reports.push(employee)}
	fire(index) {this.reports.splice(index, 1)}
}

Manager('John', 'Fisher', 'j@gmail.com', 15)
...
class Machine {
	...
	makeWidgets(num){widgets_made_count+=num;wear_and_tear_count+=num%50}
	fixMachine(){needs_reboot=true}
	reboot(){() => {wear_and_tear_count -= 10; needs_reboot=false}}
}
