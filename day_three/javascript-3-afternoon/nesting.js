...
const employeeUpdater = () => {
    ...
    if(e['firstName'] === 'Theo') employees.splice(ind, e);
    if(e['firstName']==='Lorie')e['department'] = 'HR'})
	return employees;
}
...
const removeDuplicates = () {
	return workplaceAccidents.filter((val, ind) => workplaceAccidents.indexOf(val) == ind)
}
...
const looper = () => {
	return numsArr.map((let item) => { return item.map((let val, let ind) => { return val%2==0?item[ind]='even':item[ind]='odd'}}
}
